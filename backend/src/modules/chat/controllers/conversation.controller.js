...
exports.createConversation = async (req, res) => {
  try {
    ...
    if (!receiverId) return res.status(400).json({ message: "The Person you try to chat with does not exist" });
...
exports.makeGroupChat = async (req, res) => {
  try {
    ...
    const populatedGroup = await conversationService.createGroup({ groupName, participants, adminId: userId });
...
exports.addMembersToGroupChat = async (req, res) => {
  try {
    ...
    if (!Array.isArray(members) || members.length === 0) return res.status(400).json({ message: "Members must be a non-empty array" });
    ...
    try {
      ...
      if (err.message === "NOT_FOUND") return res.status(404).json({ message: "Conversation not found" });
      if (err.message === "NOT_A_GROUP") return res.status(400).json({ message: "This conversation is not a group chat" });
      if (err.message === "NOT_A_MEMBER") return res.status(403).json({ message: "You are not a member of this group" });
      if (err.message === "NOT_AUTHORIZED_ADMIN") return res.status(403).json({ message: "Only group admins can add members" });
    }
    ...
    if (newMembers.length === 0) return res.status(400).json({ message: "All provided users are already in this group" });

    const updatedConversation = await Conversation.findByIdAndUpdate(
      conversationId,
      {
        $addToSet: { participants: { $each: newMembers } },
        $push: { unreadCounts: { $each: newMembers.map((id) => ({ user: id, count: 0 })) } },
      },
      { new: true }
    )
    .populate("participants", "firstName lastName username avatar")
...
exports.removeMemberFromGroupChat = async (req, res) => {
  try {
    ...
    if (!conversationId || !memberIdToRemove) return res.status(400).json({ message: "Conversation ID and Member ID to remove are required" });
    ...
    try {
      ...
      if (err.message === "NOT_A_GROUP") return res.status(400).json({ message: "This conversation is not a group chat" });
      if (err.message === "NOT_A_MEMBER") return res.status(403).json({ message: "You are not a member of this group" });
      if (err.message === "NOT_AUTHORIZED_ADMIN") return res.status(403).json({ message: "Only group admins can remove members from the chat" });
    }

    const isMemberToRemove = conversation.participants.some(partId => partId.toString() === memberIdToRemove.toString());
    if (!isMemberToRemove) return res.status(400).json({ message: "The user is not a member of this group" });
...
exports.leaveGroupChat = async (req, res) => {
  try {
    ...
    if (!conversation.isGroup) return res.status(400).json({ message: "You can only leave group chats" });
    ...
    return res.status(200).json({ message: "You have left the group successfully", conversation: updatedConversation });
  } catch (err) {
...
exports.addGroupAdmin = async (req, res) => {
  try {
    ...
    if (!conversationId || !targetUserId) return res.status(400).json({ message: "Conversation ID and Target User ID are required" });
    ...
    try {
      ...
      if (err.message === "NOT_A_GROUP") return res.status(400).json({ message: "This conversation is not a group chat" });
      if (err.message === "NOT_A_MEMBER") return res.status(403).json({ message: "You are not a member of this group" });
      if (err.message === "NOT_AUTHORIZED_ADMIN") return res.status(403).json({ message: "Only current group admins can assign new admins" });
    }

    const isMember = conversation.participants.some(id => id.toString() === targetUserId.toString());
    if (!isMember) return res.status(400).json({ message: "User must be a member of the group first" });

    const isAlreadyAdmin = conversation.groupAdmin.some(id => id.toString() === targetUserId.toString());
    if (isAlreadyAdmin) return res.status(400).json({ message: "User is already an admin in this group" });
...
exports.updateGroupDetails = async (req, res) => {
  try {
    ...
    const { isAdmin } = conversationService.validateGroupAndAdmin(conversation, userId);
    ...
    if (adminPermission !== undefined) {
      if (!isAdmin) return res.status(403).json({ message: "Only admins can change group permission settings" });
      updateData.adminPermission = adminPermission;
    }

    if (Object.keys(updateData).length === 0) return res.status(400).json({ message: "No data provided for update" });
...
exports.demoteAdmin = async (req, res) => {
  try {
    ...
    try {
      ...
      if (err.message === "NOT_FOUND") return res.status(404).json({ message: "Group not found" });
      if (err.message === "NOT_A_GROUP") return res.status(400).json({ message: "This is not a group chat" });
      if (err.message === "NOT_AUTHORIZED_ADMIN") return res.status(403).json({ message: "Only group admins can demote other admins" });
    }

    const isTargetAdmin = conversation.groupAdmin.some(id => id.toString() === targetUserId.toString());
    if (!isTargetAdmin) return res.status(400).json({ message: "User is not an admin in this group" });
